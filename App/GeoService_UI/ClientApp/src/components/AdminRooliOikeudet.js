...
import AdminRooliOikeudetPreview from "./AdminRooliOikeudetPreview";
...
export class AdminRooliOikeudet extends Component {
                        ...
                        authFetch(this.props.pca, API_PREFIX + '/Read')
                            ...
                            .then(data => {
                                ...
                                authFetch(this.props.pca, PROFIILI_GET)
                                    .then(response => response.json())
                                    .then(data => {
                                        var cols = (((JSON.parse(data.uI_Settings) || {}).columns || {}).roolioikeudet || []);
    ...
    addNewLookupItem = (list, item) => {
        if (list.length > 0 && item.length > 0) {
            ...
            /*
            if (list === "entiteetit") {
                fetch(API_ENTITEETIT_PREFIX + '/CreateAddHoc', options);
            }
            */
        }
    }
    ...
    render() {
        ...
        for (var i = 0; i < default_columns.length; i++) {
            default_columns[i].label = dictionary.AdminRooliOikeudet.Columns[i]
        }
        ...
        var options = {
            ...
            customToolbarSelect: (selectedRows, displayData, setSelectedRows) => {
                ...
                return (
                    <div className={"custom-toolbar-select"}>
                        <AdminRooliOikeudetPreview lookupLists={this.state.lookupLists} Daatta={this.state.data} rowId={rowId} />
            ...
            customToolbar: () => {
                return (
                    <React.Fragment>
                        <AdminRooliOikeudetAdd onAddNewRow={this.addNewItem} lookupLists={this.state.lookupLists} onLookupListChange={this.addNewLookupItem} />
