...
import KoodiPreview from "./KoodiPreview";
...
const API_PREFIX = 'api/Koodi';
const API_KOODIRYHMAT_PREFIX = 'api/Koodiryhma';
...
/* Luokka */
export class Koodi extends Component {
    ...
    componentDidMount() {
        ...
        //koodiryhmat
        authFetch(this.props.pca, API_KOODIRYHMAT_PREFIX + '/Read')
            .then(response => response.json())
            .then(data => {
                data.forEach(function (d, i) {
                    koodiryhmat[d.koodiryhmaAvain] = (d.koodiryhmaNimi || 'ei tietoja');
                });
                //Items
                authFetch(this.props.pca, API_PREFIX + '/Read')
                    ...
                    .then(data => {
                        ...
                        authFetch(this.props.pca, PROFIILI_GET)
                            ...
                            .then(data => {
                                console.log(data);
                                var cols = (((JSON.parse(data.uI_Settings) || {}).columns || {}).koodi || []);
    ...
    swapCols = (index) => {
        if (index > 1) {
            ...
            var setting = { "key": "columns.koodi", "value": JSON.stringify(cols.map(function (d) { return d.name; })).split('"').join('""') };
    ...
    addNewLookupItem = (list, item) => {
        if (list.length > 0 && item.length > 0) {
            ...
            if (list === "koodiryhmat") {
                authPost(this.props.pca, API_KOODIRYHMAT_PREFIX + '/CreateAddHoc', options);
            }
        }
    };
        ...
        var options = {
           ...
           responsive: "standard",
            ...
            customToolbarSelect: (selectedRows, displayData, setSelectedRows) => {
                ...
                return (
                    <div className={"custom-toolbar-select"}>
                        <KoodiPreview Daatta={this.state.data} lookupLists={this.state.lookupLists} rowId={rowId} />
                        <KoodiEdit onEditRow={this.editItem} lookupLists={this.state.lookupLists} onLookupListChange={this.addNewLookupItem} Daatta={this.state.data} rowId={rowId} />
            ...
            customToolbar: () => {
                return (
                    <React.Fragment>
                        <KoodiAdd onAddNewRow={this.addNewItem} lookupLists={this.state.lookupLists} onLookupListChange={this.addNewLookupItem} />
