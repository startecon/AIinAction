...
import AdminRooliAdd from "./AdminRooliAdd";
import AdminRooliEdit from "./AdminRooliEdit";
import AdminRooliPreview from "./AdminRooliPreview";
...
/* Luokka */
export class AdminRooli extends Component {
        ...
        authFetch(this.props.pca, API_PREFIX + '/Read')
            ...
            .then(data => {
                ...
                authFetch(this.props.pca, PROFIILI_GET)
                    .then(response => response.json())
                    .then(data => {
                        var cols = (((JSON.parse(data.uI_Settings) || {}).columns || {}).rooli || []);
    ...
    swapCols = (index) => {
        if (index > 1) {
            ...
            var setting = { "key": "columns.rooli", "value": JSON.stringify(cols.map(function (d) { return d.name; })).split('"').join('""') };
    ...
    render() {
        ...
        for (var i = 0; i < default_columns.length; i++) {
            default_columns[i].label = dictionary.AdminRooli.Columns[i]
        }
        ...
        var options = {
            ...
            customToolbarSelect: (selectedRows, displayData, setSelectedRows) => {
                ...
                return (
                    <div className={"custom-toolbar-select"}>
                        <AdminRooliPreview Daatta={this.state.data} rowId={rowId} />
                        <AdminRooliEdit onEditRow={this.editItem} lookupLists={this.state.lookupLists} Daatta={this.state.data} rowId={rowId} />
            ...
            customToolbar: () => {
                return (
                    <React.Fragment>
                        <AdminRooliAdd onAddNewRow={this.addNewItem} lookupLists={this.state.lookupLists} onLookupListChange={this.addNewLookupItem} />
            ...
            <div className="Cont">
                <div>
                    {(this.state.loading) ? (
                        ...
                        <MUIDataTable
                            title={dictionary.AdminRooli.Title}
